...
{
    if (selectedIndex < 0 || selectedIndex >= _itemViews.count)
    {
        return;
    }
    ...
    if (animated) 
    {
        if ([self.delegate respondsToSelector:@selector(panelBar:canSwitchFromIndex:toIndex:)])
        {
            BOOL canSwitch = [self.delegate panelBar:self canSwitchFromIndex:fromIndex toIndex:selectedIndex];
            if (!canSwitch)
            {
                return;
            }
    ...
    if (animated)
    {
        if ([self.delegate respondsToSelector:@selector(panelBar:didSwitchFromIndex:toIndex:)])
        {
            [self.delegate panelBar:self didSwitchFromIndex:fromIndex toIndex:selectedIndex];
        }
    }
